...
class OnlineManager(Serializable):
    ...
    def prepare_signals(self, prepare_func: Callable = AverageEnsemble(), over_write=False):
        ...
        if old_signals is not None and not over_write:
            ...
            if len(new_signals) > 1:
                new_signals = new_signals.iloc[1:]
    ...
    def get_portfolio(self, port_ana_config):
        strategy_config = port_ana_config["strategy"]
        backtest_config = port_ana_config["backtest"]
        strategy = init_instance_by_config(strategy_config, accept_types=BaseStrategy)

        report_dict = normal_backtest(self.signals, strategy=strategy, **backtest_config)
        ...
        return positions_normal, analysis_df
